...
const ButtonComponent = forwardRef(
  (
    { children, sx: sxProp = defaultSxProp, ...props },
    forwardedRef
  ): JSX.Element => {
    let sxStyles = sxProp;
    ...
    if (sxProp !== null && Object.keys(sxProp).length > 0) {
      sxStyles = generateCustomSxProp(
        { block, size, leadingIcon, trailingIcon, trailingAction },
        sxProp
      );
    }
...
export function generateCustomSxProp(
  props: Partial<
    Pick<
      ButtonProps,
      "size" | "block" | "leadingIcon" | "trailingIcon" | "trailingAction"
    >
  >,
  ...
  const noVisuals =
    props.leadingIcon || props.trailingIcon || props.trailingAction
      ? ""
