...
This hooks can get the theme states of numen within any component, and can track changes in states.
In the default scenario (you haven't changed the `themeType`), hooks gets the same value as `Themes.getPresets`.

Usually the components from numen are already added with theme states, you don't need to change it,
this hooks just helps you to build your own components using numen's themes.

Notice in the example below that after we define the component using Theme,
the properties of the custom component will change according to the current numen's theme,
...
```ts
const theme: numenUIThemes = useTheme()

export type numenUIThemes = {
  type: string
...
const myFontTheme: numenUIThemesFont = {}
```
