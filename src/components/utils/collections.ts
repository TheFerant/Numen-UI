...
export const isnumenElement = (el?: HTMLElement): boolean => {
  if (!el) return false
  if (el?.dataset && el?.dataset['numen']) return true
  el.attributes.getNamedItem('data-numen')
  return !!el.attributes.getNamedItem('data-numen')
}
