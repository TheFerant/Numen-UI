...
export type numenNamedContext<T, N> = {
  [key in string as `use${Capitalize<string & N>}Context`]: () => T
}

export type numenNamedProvider<T, N> = {
  [key in string as `${Capitalize<string & N>}Provider`]: T
}

export const createnumenContext = <S extends Record<string, unknown>, N extends string>(
  name: N,
  initialStates: S,
) => {
  ...
  type numenContextProps = {
    defaultValues?: Partial<S> | (() => Partial<S>)
    onChange?: ContextStateOnChange<S>
    onChangeBefore?: ContextStateFilter<S>
  }

  const numenContext = React.forwardRef<S, React.PropsWithChildren<numenContextProps>>(
  ...
  type ResultType = numenNamedProvider<typeof numenContext, N> &
    numenNamedContext<ContextStates<S>, N>

  return {
    [`${capitalize(name)}Provider`]: numenContext,
